...
class SimpleTrainer(TrainerBase):
    ...
    def __init__(self, cfg):
        ...
        #tag = self.cfg.TRAINER.NAME + " / " + self.cfg.DATASET.NAME + "/"
        tag = self.cfg.DATASET.NAME + "/"
        '''
        for source in self.cfg.DATASET.SOURCE_DOMAINS:
            tag = tag + " " + source
        tag = tag + " /"
        '''
        for target in self.cfg.DATASET.TARGET_DOMAINS:
            tag = tag + target
        self.tag = tag
    ...
    def test(self, split=None):
        ...
        results = self.evaluator.evaluate(verbose=True)

        '''
        for k, v in results.items():
            tag = self.tag + "/test_" + k
            self.write_scalar(tag, v, batch_idx + 10)
        '''
...
class TrainerX(SimpleTrainer):
    ...
    def run_epoch(self):
        ...
        for self.batch_idx, batch in enumerate(self.train_loader_x):
            ...
            '''
            for name, meter in losses.meters.items():
                self.write_scalar(self.tag + "/train_" + name, meter.avg, n_iter)
            '''

            end = time.time()
        
        self.in_training_test(self.epoch+1)
    ...
    def in_training_test(self, n_iter, split=None):
        self.set_model_mode("eval")
        self.evaluator.reset()
        ...
        for k, v in results.items():
            tag = self.tag + "/test_" + k
            self.write_scalar(tag, v, n_iter)
